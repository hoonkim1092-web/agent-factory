<#
.SYNOPSIS
Agent Factory CLI v1.2.27 설치 스크립트

.EXAMPLE
irm https://raw.githubusercontent.com/hoonkim1092-web/af-fsa/af-fsa_v1.2.27/install-af.ps1 | iex
#>
...
if (-not $isAdmin) {
    ...
    $cmdline = "-ExecutionPolicy Bypass -Command `"& { `$tmp = [IO.Path]::GetTempFileName() + '.ps1'; try { irm https://raw.githubusercontent.com/hoonkim1092-web/af-fsa/af-fsa_v1.2.27/install-af.ps1 -OutFile `$tmp; & `$tmp${extraArgs} } finally { Remove-Item `$tmp -ErrorAction SilentlyContinue } }`""
...
Write-Host "  Agent Factory CLI v1.2.27 설치" -ForegroundColor Cyan
Write-Host "============================================================" -ForegroundColor Cyan
...
# Step 3: 다운로드
$zipFile = "$InstallPath\af-1.2.27.zip"
Write-Host "`n► af-1.2.27.zip 다운로드 중..." -ForegroundColor Yellow
try {
    $ProgressPreference = 'SilentlyContinue'
    Invoke-WebRequest `
        -Uri "https://github.com/hoonkim1092-web/af-fsa/raw/af-fsa_v1.2.27/dist/af-1.2.27.zip" `
        -OutFile $zipFile `
        -UseBasicParsing
...
Write-Host ""
Write-Host "============================================================" -ForegroundColor Cyan
Write-Host "  설치 완료! v1.2.27" -ForegroundColor Green
Write-Host "============================================================" -ForegroundColor Cyan
Write-Host ""
